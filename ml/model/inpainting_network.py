"""
TPS Motion Model — Inpainting Network
Encoder-decoder that warps source image features using optical flow,
applies occlusion masks, and reconstructs the animated frame.
...
from ml.model.util import ResBlock2d, SameBlock2d, UpBlock2d, DownBlock2d


class InpaintingNetwork(nn.Module):
    """
    Inpaints missing regions and reconstructs the driving image.
    
    Args:
        num_channels (int): Input image channels (3 for RGB)
        block_expansion (int): Base channel count
        max_features (int): Maximum channel count
        num_down_blocks (int): Number of downsampling blocks
        multi_mask (bool): Use multi-resolution occlusion masks
    """

    def __init__(self, num_channels, block_expansion, max_features,
                 num_down_blocks, multi_mask=True, **kwargs):
        super(InpaintingNetwork, self).__init__()

        self.num_down_blocks = num_down_blocks
        self.multi_mask = multi_mask
        self.first = SameBlock2d(
            num_channels, block_expansion,
        ...
        for i in range(num_down_blocks):
            ...
            up_blocks.append(UpBlock2d(
                decoder_in_feature, in_features,
                kernel_size=(3, 3), padding=(1, 1)
            ))
        ...
        self.final = nn.Conv2d(
            block_expansion, num_channels,
            kernel_size=(7, 7), padding=(3, 3)
        )
        self.num_channels = num_channels

    def deform_input(self, inp, deformation):
        """Warp input features using deformation field."""
        _, h_old, w_old, _ = deformation.shape
        _, _, h, w = inp.shape
        if h_old != h or w_old != w:
            deformation = deformation.permute(0, 3, 1, 2)
            deformation = F.interpolate(
                deformation, size=(h, w),
    ...
    def occlude_input(self, inp, occlusion_map):
        """Apply occlusion mask to input features."""
        if not self.multi_mask:
            if (inp.shape[2] != occlusion_map.shape[2] or
                    inp.shape[3] != occlusion_map.shape[3]):
                occlusion_map = F.interpolate(
                    occlusion_map, size=inp.shape[2:],
                    mode='bilinear', align_corners=True
                )
    ...
    def forward(self, source_image, dense_motion):
        ...
        Args:
            ...
            dense_motion: dict from DenseMotionNetwork with
                'deformation', 'occlusion_map', etc.
                
        Returns:
            dict with 'prediction' (animated frame), 'deformed',
            'warped_encoder_maps', occlusion/contribution info
        """
        ...
        if not self.multi_mask:
            occlusion_last = F.interpolate(
                occlusion_last, size=out.shape[2:],
                mode='bilinear', align_corners=True
            )
    ...
    def get_encode(self, driver_image, occlusion_map):
        """Encode driver image features for cross-identity animation."""
        out = self.first(driver_image)
        encoder_map = []
        encoder_map.append(
            self.occlude_input(out.detach(), occlusion_map[-1].detach())
        )
        for i in range(len(self.down_blocks)):
