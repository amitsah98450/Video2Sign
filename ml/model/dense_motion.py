"""
TPS Motion Model — Dense Motion Network
Estimates optical flow and multi-resolution occlusion masks from
K TPS transformations and an affine background transformation.
...
import torch
from ml.model.util import (
    Hourglass, AntiAliasInterpolation2d, make_coordinate_grid,
    kp2gaussian, to_homogeneous, from_homogeneous, UpBlock2d, TPS
)
import math
...
class DenseMotionNetwork(nn.Module):
    """
    Estimates optical flow and multi-resolution occlusion masks
    from K TPS transformations and an affine transformation.
    
    Args:
        block_expansion (int): Base channel count for Hourglass
        num_blocks (int): Number of encoder/decoder blocks in Hourglass
        max_features (int): Maximum channel count
        num_tps (int): Number of TPS transformations
        num_channels (int): Input image channels (3 for RGB)
        scale_factor (float): Downscale factor for processing
        bg (bool): Whether to use background motion
        multi_mask (bool): Whether to use multi-resolution occlusion
        kp_variance (float): Variance for keypoint Gaussians
    """

    def __init__(self, block_expansion, num_blocks, max_features, num_tps,
                 num_channels, scale_factor=0.25, bg=False,
                 multi_mask=True, kp_variance=0.01):
        ...
        self.hourglass = Hourglass(
            ...
            in_features=(
                num_channels * (num_tps + 1) + num_tps * 5 + 1
            ),
        ...
        self.maps = nn.Conv2d(
            hourglass_output_size[-1], num_tps + 1,
    ...
    def create_heatmap_representations(self, source_image, kp_driving,
                                       kp_source):
        """Create difference heatmaps from keypoint Gaussians."""
        spatial_size = source_image.shape[2:]
        gaussian_driving = kp2gaussian(
            kp_driving['fg_kp'], spatial_size=spatial_size,
            kp_variance=self.kp_variance
        )
        gaussian_source = kp2gaussian(
            kp_source['fg_kp'], spatial_size=spatial_size,
            kp_variance=self.kp_variance
        )
    ...
    def create_transformations(self, source_image, kp_driving,
                               kp_source, bg_param):
        """Create K TPS transformations + 1 background affine."""
        ...
        if bg_param is not None:
            ...
            identity_grid = torch.matmul(
                bg_param.view(bs, 1, 1, 1, 3, 3),
                identity_grid.unsqueeze(-1)
            ).squeeze(-1)
    ...
    def create_deformed_source_image(self, source_image, transformations):
        """Warp source image using all transformations."""
        bs, _, h, w = source_image.shape
        source_repeat = source_image.unsqueeze(1).unsqueeze(1).repeat(
            1, self.num_tps + 1, 1, 1, 1, 1
        )
        source_repeat = source_repeat.view(
            bs * (self.num_tps + 1), -1, h, w
        )
        transformations = transformations.view(
            (bs * (self.num_tps + 1), h, w, -1)
        )
        deformed = F.grid_sample(
            source_repeat, transformations, align_corners=True
        )
    ...
    def forward(self, source_image, kp_driving, kp_source,
                bg_param=None, dropout_flag=False, dropout_p=0):
        """
        Args:
            source_image: (B, 3, H, W)
            kp_driving: dict with 'fg_kp' (B, K*5, 2)
            kp_source: dict with 'fg_kp' (B, K*5, 2)
            bg_param: optional (B, 3, 3) affine matrix
            dropout_flag: whether to apply TPS dropout
            dropout_p: dropout probability
            
        Returns:
            dict with 'deformation', 'occlusion_map', 
            'contribution_maps', 'deformed_source'
        """
        ...
        input = torch.cat(
            [heatmap_representation, deformed_source], dim=1
        )
        ...
        if dropout_flag:
            contribution_maps = self.dropout_softmax(
                contribution_maps, dropout_p
            )
        else:
            contribution_maps = F.softmax(contribution_maps, dim=1)
        out_dict['contribution_maps'] = contribution_maps

        # Combine K+1 transformations — Eq(6) in the paper
        ...
        if self.multi_mask:
            for i in range(self.occlusion_num - self.up_nums):
                occlusion_map.append(torch.sigmoid(
                    self.occlusion[i](
            ...
            for i in range(self.up_nums):
                ...
                occlusion_map.append(torch.sigmoid(
                    self.occlusion[
                        i + self.occlusion_num - self.up_nums
                    ](prediction)
                ))
        else:
            occlusion_map.append(
                torch.sigmoid(self.occlusion[0](prediction[-1]))
            )
