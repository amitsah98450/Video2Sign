"""
TPS Motion Model — Utility Modules
Core building blocks: TPS transformation, Hourglass, ResBlock2d, 
Up/DownBlock2d, AntiAliasInterpolation2d, coordinate grid helpers.

Based on: "Thin-Plate Spline Motion Model for Image Animation" (CVPR 2022)
Reference: https://github.com/yoyo-nb/Thin-Plate-Spline-Motion-Model
"""
...
class TPS:
    """
    TPS transformation.
    Mode 'kp': Eq(2) in the paper — warp using source/driving keypoint pairs.
    Mode 'random': Random TPS for equivariance loss during training.
    """
    ...
    def __init__(self, mode, bs, **kwargs):
        ...
        if mode == 'random':
            noise = torch.normal(
                mean=0,
                std=kwargs['sigma_affine'] * torch.ones([bs, 2, 3])
            )
            ...
            self.control_params = torch.normal(
                mean=0,
                std=kwargs['sigma_tps'] * torch.ones(
    ...
    def warp_coordinates(self, coordinates):
        ...
        if self.mode == 'kp':
            transformed = torch.matmul(
                theta[:, :, :, :2], coordinates.permute(0, 2, 1)
            ) + theta[:, :, :, 2:]
            ...
            result = torch.matmul(
                result.permute(0, 1, 3, 2), control_params
            )
            ...
            transformed = torch.matmul(
                theta[:, :, :, :2], coordinates.unsqueeze(-1)
            ) + theta[:, :, :, 2:]
...
class ResBlock2d(nn.Module):
    """Res block, preserves spatial resolution."""
...
class SameBlock2d(nn.Module):
    """Simple block, preserves spatial resolution."""
...
class Encoder(nn.Module):
    """Hourglass Encoder — progressively downsamples."""
...
class Decoder(nn.Module):
    """Hourglass Decoder — progressively upsamples with skip connections."""
...
class AntiAliasInterpolation2d(nn.Module):
    """Band-limited downsampling for better signal preservation."""
