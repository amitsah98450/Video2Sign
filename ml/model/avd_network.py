"""
TPS Motion Model — Animation via Disentanglement (AVD) Network
Separates identity (shape) from pose to enable cross-identity animation.
...
class AVDNetwork(nn.Module):
    ...
    Encodes source keypoints into an identity representation and
    driving keypoints into a pose representation, then decodes
    combined features to produce new keypoints preserving source
    shape with driving pose.
    
    Args:
        num_tps (int): Number of TPS transformations
        id_bottle_size (int): Identity bottleneck dimension
        pose_bottle_size (int): Pose bottleneck dimension
    """
    ...
    def forward(self, kp_source, kp_random):
        """
        Args:
            kp_source: dict with 'fg_kp' (B, K*5, 2) — identity source
            kp_random: dict with 'fg_kp' (B, K*5, 2) — pose source
            
        Returns:
            dict with 'fg_kp' — reconstructed keypoints
        """
